import Link from "next/link";
import { notFound } from "next/navigation";
import { Badge } from "@/components/ui/badge";
import { Card, CardContent } from "@/components/ui/card";
import { AskQuestionButton } from "@/components/socionics/ask-question";
import { auth } from "@/lib/auth/auth";
import { isLocale } from "@/lib/i18n/config";
import { getDictionary } from "@/lib/i18n/dictionaries";
import { api } from "@/lib/trpc/server";

export const dynamic = "force-dynamic";

export default async function QuestionsPage({
  params,
}: {
  params: Promise<{ lang: string }>;
}) {
  const { lang } = await params;
  if (!isLocale(lang)) notFound();
  const dict = getDictionary(lang);
  const session = await auth();
  const isAuthed = Boolean(session?.user);
  const list = await api.question.list({ language: lang, limit: 50 });

  return (
    <div className="mx-auto max-w-3xl px-6 py-12 space-y-6">
      <header className="flex items-start justify-between gap-3 flex-wrap">
        <div className="space-y-2 max-w-xl">
          <h1 className="font-heading text-3xl font-semibold tracking-tight">
            {dict.questions.title}
          </h1>
          <p className="text-sm text-muted-foreground">
            {dict.questions.subtitle}
          </p>
        </div>
        {isAuthed ? (
          <AskQuestionButton lang={lang} dict={dict} />
        ) : (
          <Link
            href={`/${lang}/login?callbackUrl=/${lang}/questions`}
            className="text-sm rounded-md border border-border px-3 py-1.5 hover:bg-muted transition-colors"
          >
            {dict.questions.loginToAsk}
          </Link>
        )}
      </header>

      {list.length === 0 ? (
        <Card>
          <CardContent className="py-8 text-center text-sm text-muted-foreground">
            {dict.questions.empty}
          </CardContent>
        </Card>
      ) : (
        <ul className="space-y-2">
          {list.map((q) => (
            <li key={q.id}>
              <Link
                href={`/${lang}/questions/${q.slug}`}
                className="block rounded-md border border-border px-3 py-2.5 hover:bg-muted/40 transition-colors space-y-1"
              >
                <div className="flex items-center gap-2 flex-wrap">
                  {q.isResolved && (
                    <Badge className="text-xs bg-emerald-600 text-white hover:bg-emerald-600">
                      {dict.questions.resolved}
                    </Badge>
                  )}
                  <span className="text-xs text-muted-foreground font-mono">
                    {q.answerCount} {dict.questions.answersShort}
                  </span>
                  {q.author && (
                    <span className="text-xs text-muted-foreground ml-auto">
                      @{q.author.username}
                    </span>
                  )}
                </div>
                <h3 className="text-base font-medium">{q.title}</h3>
                <p className="text-sm text-muted-foreground line-clamp-2 leading-relaxed">
                  {q.body}
                </p>
              </Link>
            </li>
          ))}
        </ul>
      )}
    </div>
  );
}
